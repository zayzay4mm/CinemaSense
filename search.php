...
$next = $page;
if (isset($_GET['q'])) {
   $text = urldecode($_GET['q']);
   $text = mysqli_real_escape_string($conn,$text);
   //total
   $sql = "select count(*) as total from posts where title like '$text%'";
   ...
   //return count
   $sql = "select * from posts where title like '$text%' limit 0,$end";
   $result = $conn->query($sql);
   if ($result->num_rows >= 0) {
   ...
   <div class='d-flex justify-content-between align-items-center mark-heading'>
      <span class='ms-1'> Results for : $text </span><span class='badge rounded-pill bg-danger text-white'>$count / $total</span>
   </div>
   <div class='container-fluid'>
   <div class='row'>";
    $sql = "select * from posts where title like '$text%' order by id desc limit $start,$limit";
     $result = $conn->query($sql);
     if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          $id = $row["id"];
     ...
     <div class='card shadow overflow-hidden'>
       ...
       <div class='d-flex justify-content-between align-items-center'>
         <div class=''>
           <img class='logo' src='assets/images/profile/$profile'/><span class='ms-2 post-author'>$author</span>
         </div>
         <div class=''>";
         for ($i=0; $i < 5; $i++) {
             if ($i >= $rating) {
       ...
       <div class='d-flex flex-column min-vh-100 align-items-center justify-content-center'>
         <i class='fa-regular fa-face-frown search-warning-emoji'></i>
         <span class='search-warning-text'>Sorry,no results found !</span>
       </div>
       ";
     }
 echo "</div></div></div>";
...
echo "
  ...
  <div class='d-flex justify-content-between align-items-center'>";
  if ($count > $limit) {
    echo "<a class='btn btn-outline-secondary opacity-100' href='search.php?q=$text&page=$previous'><i class='fa-solid fa-chevron-left'></i> Previous</a>";
  } else {
    echo "<div></div>";
  }
  if ($count >= $total) {
     ...
     echo "<a class='btn btn-outline-secondary opacity-100' href='search.php?q=$text&page=$next'>Next <i class='fa-solid fa-chevron-right'></i></a>";
  }
echo "</div></div>";
?>
